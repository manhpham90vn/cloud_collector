...
impl ResourceCollector for CloudFormationCollector {
    async fn collect(&self, cli: &AwsCli, region: &str) -> Result<Vec<ResourceCollection>> {
        ...
        let commands = vec![
            ("stacks", vec!["cloudformation", "describe-stacks", "--region", region]),
            ("stack-sets", vec!["cloudformation", "list-stack-sets", "--region", region]),
            ("exports", vec!["cloudformation", "list-exports", "--region", region]),
        ];
