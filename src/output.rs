...
use crate::models::{ResourceCollection, Metadata};
...
fn write_files(collections: &[ResourceCollection], output_dir: &str, create_new_file: bool) -> Result<()> {
    ...
    for collection in collections {
        ...
        grouped.entry(key).or_insert_with(Vec::new).push(collection);
    }
    ...
    for ((service, region), group) in grouped {
        let filename = if create_new_file {
            format!("{}_{}_all_{}.json", service, region, chrono::Utc::now().format("%Y%m%d_%H%M%S"))
        } else {
        ...
        println!("✓ Wrote: {} ({} resource types)", filepath.display(), group.len());
    }
